...
void Fsm::add_transition(State* state_from, State* state_to, int event, void (*on_transition)()){
	if ( state_from == NULL )  return;
	if( state_to == NULL ) state_to = state_from;  
...
void Fsm::run_machine(){
	...
	if (!m_initialized){
		m_initialized = true;
		if (m_current_state->on_enter != NULL)	m_current_state->on_enter();
  	}
  
	if (m_current_state->on_state != NULL) m_current_state->on_state();
	Fsm::check_timed_transitions();
}

void Fsm::make_transition(Transition* transition){
	// Execute the handlers in the correct order.
	if (transition->on_transition != NULL) transition->on_transition();
	if( m_current_state != transition->state_to ){
		if (transition->state_from->on_exit != NULL)  transition->state_from->on_exit();
		if (transition->state_to->on_enter != NULL) transition->state_to->on_enter();
	}
